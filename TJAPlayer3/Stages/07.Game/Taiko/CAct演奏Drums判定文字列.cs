...
namespace TJAPlayer3
{
	internal class CAct演奏Drums判定文字列 : CActivity
	{
        ...
        public override int On進行描画()
        {
            if (!base.b活性化してない)
            {
                for (int i = 0; i < 512; i++)
                {
                    for (int j = 0; j < 2; j++)
                    {
                        var judgeAnime = JudgeAnimes[j, i];
                        if (judgeAnime.counter.b停止中) continue;
                        judgeAnime.counter.t進行();
                        float x = TJAPlayer3.Skin.nScrollFieldX[0] - TJAPlayer3.Tx.Judge.szテクスチャサイズ.Width / 2;
                        float y = (TJAPlayer3.Skin.nScrollFieldY[j] - 53 + CubicEaseOut((float)(judgeAnime.counter.n現在の値 >= 180 ? 180 : judgeAnime.counter.n現在の値) / 180f) * 20f) - 10;
                        TJAPlayer3.Tx.Judge.Opacity = (int)(255f - (judgeAnime.counter.n現在の値 >= 360 ? ((judgeAnime.counter.n現在の値 - 360) / 50.0f) * 255f : 0f));
                        TJAPlayer3.Tx.Judge.t2D描画(TJAPlayer3.app.Device, x, y, judgeAnime.rc);
                    }
                }
            }
            return 0;
        ...
        {
            var judgeAnime = JudgeAnimes[player, JudgeAnime.Index];
            judgeAnime.counter.t開始(0, 410, 1, TJAPlayer3.Timer);
            judgeAnime.Judge = judge;
            int njudge = judge == E判定.Perfect ? 0 : judge == E判定.Good ? 1 : judge == E判定.Auto ? 0 : 2;
            judgeAnime.rc = new Rectangle(0, (int)njudge * 60, 90, 60);
            JudgeAnime.Index++;
