        ...
        public override int On進行描画()
        {
            ...
            if (this.r現在選択中の曲 == null)
            {
				...
				*/
                ...
                var targetTexture = bIsEnumeratingSongs ? txEnumeratingSongs : txSongNotFound;
                targetTexture?.t2D描画(TJAPlayer3.app.Device, 320, 160);
            ...
            for (int i = 0; i < 9; i++) // パネルは全13枚。
            {
                ...
                #region [ バーテクスチャを描画。]
                //-----------------
                TJAPlayer3.Tx.SongSelect_Bar_Shadow.vc拡大縮小倍率.X = 1.0f;
                ...
                //選択されていない曲名の座標
                if (n現在のスクロールカウンタ != 0)
                    ResolveTitleTexture(this.stバー情報[nパネル番号].ttkタイトル).t2D拡大率考慮中央基準描画(TJAPlayer3.app.Device, xAnime + 316 - Box + TJAPlayer3.Skin.SongSelect_Title_X, y + 65 - (Box * 3) + TJAPlayer3.Skin.SongSelect_Title_Y);//59
                else if (n見た目の行番号 != 4)
                    ResolveTitleTexture(this.stバー情報[nパネル番号].ttkタイトル).t2D拡大率考慮中央基準描画(TJAPlayer3.app.Device, xAnime + 316 - Box + TJAPlayer3.Skin.SongSelect_Title_X, y + 65 - (Box * 3) + TJAPlayer3.Skin.SongSelect_Title_Y);//62
                #endregion
            ...
            if (this.n現在のスクロールカウンタ == 0)
            {
                ...
                if (r現在選択中の曲.eノード種別 == C曲リストノード.Eノード種別.BOX)//フォルダのトップ用の影
                {
                    #region [ BoxShadow ]

                    if (ctBoxOpen.n現在の値 >= 1300 && ctBoxOpen.n現在の値 <= 1940)
                    {
                        TJAPlayer3.Tx.SongSelect_Box_Shadow.vc拡大縮小倍率.X = 1.0f - (float)Math.Sin(((ctBoxOpen.n現在の値 - 1300) * 0.28125f) * (Math.PI / 180)) * 1.0f;
                    }
                    else
                    {
                        TJAPlayer3.Tx.SongSelect_Box_Shadow.vc拡大縮小倍率.X = 1.0f;
                    }
                    TJAPlayer3.Tx.SongSelect_Box_Shadow?.t2D拡大率考慮中央基準描画(TJAPlayer3.app.Device, 640 + xp, 326 + xp - BarAnimeCount, new Rectangle(0, 0, 632, 20));

                    TJAPlayer3.Tx.SongSelect_Box_Shadow.vc拡大縮小倍率.Y = BarAnimeCount == 0 ? 1.0f : 1.0f + (float)(BarAnimeCount) / 27.0f;
                    TJAPlayer3.Tx.SongSelect_Box_Shadow?.t2D拡大率考慮中央基準描画(TJAPlayer3.app.Device, 640 + xp, 360 + xp, new Rectangle(0, 20, 632, 52));
                    TJAPlayer3.Tx.SongSelect_Box_Shadow.vc拡大縮小倍率.Y = 1.0f;

                    TJAPlayer3.Tx.SongSelect_Box_Shadow?.t2D拡大率考慮中央基準描画(TJAPlayer3.app.Device, 640 + xp, 394 + xp + BarAnimeCount, new Rectangle(0, 72, 632, 20));

                    #endregion
                }

                if (r現在選択中の曲.eノード種別 == C曲リストノード.Eノード種別.SCORE)//フォルダ内のBarの影
                {
                    #region [ BarShadow ]

                    if (ctBoxOpen.n現在の値 >= 1300 && ctBoxOpen.n現在の値 <= 1940)
                    {
                        TJAPlayer3.Tx.SongSelect_Box_Shadow.vc拡大縮小倍率.X = 1.0f - (float)Math.Sin(((ctBoxOpen.n現在の値 - 1300) * 0.28125f) * (Math.PI / 180)) * 1.0f;//1300
                    }
                    else
                    {
                        TJAPlayer3.Tx.SongSelect_Box_Shadow.vc拡大縮小倍率.X = 1.0f;
                    ...
                    TJAPlayer3.Tx.SongSelect_Box_Shadow.vc拡大縮小倍率.Y = BarAnimeCount == 0 ? 1.0f : 1.0f + (float)(BarAnimeCount) / 27.0f;//24.6
                    TJAPlayer3.Tx.SongSelect_Box_Shadow?.t2D拡大率考慮中央基準描画(TJAPlayer3.app.Device, 640 + xp, 360 + xp, new Rectangle(0, 20, 632, 52));
                    TJAPlayer3.Tx.SongSelect_Box_Shadow.vc拡大縮小倍率.Y = 1.0f;
                ...
                if (r現在選択中の曲.eノード種別 == C曲リストノード.Eノード種別.BACKBOX)//閉じるBarの影
                {
                    #region [ BackBox(戻る) ]
                ...
                if (r現在選択中の曲.eノード種別 == C曲リストノード.Eノード種別.SCORE)//現在フォーカス中のボックス
                {
                    #region [ Score ]
                    ...
                    TJAPlayer3.Tx.SongSelect_Bar_Genre[nStrジャンルtoNum(r現在選択中の曲.strジャンル)].vc拡大縮小倍率.Y = BarAnimeCount == 0 ? 1.0f : 1.0f + (float)(BarAnimeCount) / 23.6f;//23.0
                    ...
                    if (BarAnimeCount != 0)
                    {
                        TJAPlayer3.Tx.SongSelect_Bar_Genre_Overlay?.t2D拡大率考慮中央基準描画(TJAPlayer3.app.Device, 641, 337 - BarAnimeCount, new Rectangle(0, 0, 531, 9));

                        TJAPlayer3.Tx.SongSelect_Bar_Genre_Overlay.vc拡大縮小倍率.Y = BarAnimeCount == 0 ? 1.0f : 1.0f + (float)(BarAnimeCount) / 19.10f;//24.55
                        TJAPlayer3.Tx.SongSelect_Bar_Genre_Overlay?.t2D拡大率考慮中央基準描画(TJAPlayer3.app.Device, 641, 360, new Rectangle(0, 9, 531, 38));//359
                        TJAPlayer3.Tx.SongSelect_Bar_Genre_Overlay.vc拡大縮小倍率.Y = 1.0f;

                        TJAPlayer3.Tx.SongSelect_Bar_Genre_Overlay?.t2D拡大率考慮中央基準描画(TJAPlayer3.app.Device, 641, 384 + BarAnimeCount, new Rectangle(0, 47, 531, 9));
                    }
                    else
                    {
                        TJAPlayer3.Tx.SongSelect_Bar_Genre_Overlay?.t2D拡大率考慮中央基準描画(TJAPlayer3.app.Device, 641, 360, new Rectangle(0, 0, 531, 56));
                    }
                    #endregion
                    ...
                    if (this.r現在選択中の曲.arスコア[3] != null)
                    {
                        ...
                        TJAPlayer3.Tx.SongSelect_Crown.vc拡大縮小倍率.X = 0.93f + BarAnimeCount / 315f;//620f
                        TJAPlayer3.Tx.SongSelect_Crown.vc拡大縮小倍率.Y = 0.93f + BarAnimeCount / 315f;
                        TJAPlayer3.Tx.SongSelect_ScoreRank.vc拡大縮小倍率.X = 0.85f + BarAnimeCount / 417f;
                        TJAPlayer3.Tx.SongSelect_ScoreRank.vc拡大縮小倍率.Y = 0.85f + BarAnimeCount / 417f;
                        float Nsize = 42.80f;

                        if (!(ctBoxOpen.n現在の値 >= 1300 && ctBoxOpen.n現在の値 <= 1940))
                        {
                            if (クリア[4] != 0)
                                TJAPlayer3.Tx.SongSelect_Crown.t2D拡大率考慮中央基準描画(TJAPlayer3.app.Device, 356, 343 - BarAnimeCount / 1.1f, new RectangleF(12 * 43.2f + (クリア[4] - 1) * 43.2f, 0, 43.2f, 39));
                            else if (クリア[3] != 0 && クリア[4] == 0)
                                TJAPlayer3.Tx.SongSelect_Crown.t2D拡大率考慮中央基準描画(TJAPlayer3.app.Device, 356, 343 - BarAnimeCount / 1.1f, new RectangleF(9 * 43.2f + (クリア[3] - 1) * 43.2f, 0, 43.2f, 39));
                            else if (クリア[2] != 0 && クリア[3] == 0)
                                TJAPlayer3.Tx.SongSelect_Crown.t2D拡大率考慮中央基準描画(TJAPlayer3.app.Device, 356, 343 - BarAnimeCount / 1.1f, new RectangleF(6 * 43.2f + (クリア[2] - 1) * 43.2f, 0, 43.2f, 39));
                            else if (クリア[1] != 0 && クリア[2] == 0)
                                TJAPlayer3.Tx.SongSelect_Crown.t2D拡大率考慮中央基準描画(TJAPlayer3.app.Device, 356, 343 - BarAnimeCount / 1.1f, new RectangleF(3 * 43.2f + (クリア[1] - 1) * 43.2f, 0, 43.2f, 39));
                            else if (クリア[0] != 0 && クリア[1] == 0)
                                TJAPlayer3.Tx.SongSelect_Crown.t2D拡大率考慮中央基準描画(TJAPlayer3.app.Device, 356, 343 - BarAnimeCount / 1.1f, new RectangleF(0 * 43.2f + (クリア[0] - 1) * 43.2f, 0, 43.2f, 39));


                            if (スコアランク[4] != 0)
                                TJAPlayer3.Tx.SongSelect_ScoreRank.t2D拡大率考慮中央基準描画(TJAPlayer3.app.Device, 356, 377 - BarAnimeCount / 1.3f, new RectangleF(0, (スコアランク[4] - 1) * Nsize, 50, Nsize));//42.71
                            else if (スコアランク[3] != 0 && スコアランク[4] == 0)
                                TJAPlayer3.Tx.SongSelect_ScoreRank.t2D拡大率考慮中央基準描画(TJAPlayer3.app.Device, 356, 377 - BarAnimeCount / 1.3f, new RectangleF(0, (スコアランク[3] - 1) * Nsize, 50, Nsize));
                            else if (スコアランク[2] != 0 && スコアランク[3] == 0)
                                TJAPlayer3.Tx.SongSelect_ScoreRank.t2D拡大率考慮中央基準描画(TJAPlayer3.app.Device, 356, 377 - BarAnimeCount / 1.3f, new RectangleF(0, (スコアランク[2] - 1) * Nsize, 50, Nsize));
                            else if (スコアランク[1] != 0 && スコアランク[2] == 0)
                                TJAPlayer3.Tx.SongSelect_ScoreRank.t2D拡大率考慮中央基準描画(TJAPlayer3.app.Device, 356, 377 - BarAnimeCount / 1.3f, new RectangleF(0, (スコアランク[1] - 1) * Nsize, 50, Nsize));
                            else if (スコアランク[0] != 0 && スコアランク[1] == 0)
                                TJAPlayer3.Tx.SongSelect_ScoreRank.t2D拡大率考慮中央基準描画(TJAPlayer3.app.Device, 356, 377 - BarAnimeCount / 1.3f, new RectangleF(0, (スコアランク[0] - 1) * Nsize, 50, Nsize));
                ...
                if (r現在選択中の曲.eノード種別 == C曲リストノード.Eノード種別.BOX)
                {
                    ...
                    if (BarAnimeCount != 0)//+49
                    {
                        TJAPlayer3.Tx.SongSelect_Bar_Genre_Overlay.t2D拡大率考慮中央基準描画(TJAPlayer3.app.Device, 641, 337, new Rectangle(0, 0, 531, 9));

                        TJAPlayer3.Tx.SongSelect_Bar_Genre_Overlay.vc拡大縮小倍率.Y = BarAnimeCount == 0 ? 1.0f : 1.0f + (float)(BarAnimeCount) / 37.6f;
                        TJAPlayer3.Tx.SongSelect_Bar_Genre_Overlay.t2D拡大率考慮上中央基準描画(TJAPlayer3.app.Device, 641, 341, new Rectangle(0, 9, 531, 38));
                        TJAPlayer3.Tx.SongSelect_Bar_Genre_Overlay.vc拡大縮小倍率.Y = 1.0f;

                        TJAPlayer3.Tx.SongSelect_Bar_Genre_Overlay.t2D拡大率考慮中央基準描画(TJAPlayer3.app.Device, 641, 385 + BarAnimeCount, new Rectangle(0, 47, 531, 9));
                    }
                    else
                    {
                        TJAPlayer3.Tx.SongSelect_Bar_Genre_Overlay.t2D拡大率考慮中央基準描画(TJAPlayer3.app.Device, 641, 360, new Rectangle(0, 0, 531, 56));
                                    ...
                                    for (int i = 0; i < (int)Difficulty.Edit + 1; i++)
                                    {
                                        if (ctBarOpen.n現在の値 >= 100)
                                        {
                                            ...
                                            if (TJAPlayer3.stage選曲.r現在選択中のスコア.譜面情報.nレベル[i] >= 0)
                                            {
                                                ...
                                                //ここはフレームと★
                                                if (i == 4 && !(ctBoxOpen.n現在の値 >= 1300 && ctBoxOpen.n現在の値 <= 1940))//ここが裏鬼部分？
                                                {
                                                    ...
                                                    if (!TJAPlayer3.stage選曲.act難易度選択画面.bIsDifficltSelect)
                                                    {
                                                        ...
                                                        TJAPlayer3.Tx.SongSelect_Branch.Opacity = BarAnimeCount < 62 ? 0 : opacity;

                                                    }
                                                    else if (ctDifficultyIn.n現在の値 >= 1000)
                                                    {
                                                        TJAPlayer3.Tx.SongSelect_Frame_Score.Opacity = (int)255.0f - (ctDifficultyIn.n現在の値 - 1000);
                                                        TJAPlayer3.Tx.SongSelect_Branch.Opacity = (int)255.0f - (ctDifficultyIn.n現在の値 - 1000);
                                                    }
                                                    TJAPlayer3.Tx.SongSelect_Frame_Score.t2D下中央基準描画(TJAPlayer3.app.Device, 487 + (3 * 122) - 31, TJAPlayer3.Skin.SongSelect_Overall_Y + 463, new Rectangle(122 * i, 0, 122, 360));

                                                    //裏鬼のBranch(分岐譜面)シンボル
                                                    if (TJAPlayer3.Tx.SongSelect_Branch != null && TJAPlayer3.stage選曲.r現在選択中のスコア.譜面情報.b譜面分岐[4])
                                                        TJAPlayer3.Tx.SongSelect_Branch.t2D中心基準描画(TJAPlayer3.app.Device, 775, TJAPlayer3.Skin.SongSelect_Overall_Y + 273);

                                                }
                                                else if (i != 4 && !(ctBoxOpen.n現在の値 >= 1300 && ctBoxOpen.n現在の値 <= 1940))
                                                {
                                                    if (!TJAPlayer3.stage選曲.act難易度選択画面.bIsDifficltSelect)
                                                    {
                                                        TJAPlayer3.Tx.SongSelect_Frame_Score.Opacity = (int)(BarAnimeCount * 4.25f);
                                                        TJAPlayer3.Tx.SongSelect_Branch.Opacity = (int)(BarAnimeCount * 4.25f);
                                                    }
                                                    else if (ctDifficultyIn.n現在の値 >= 1000)
                                                    {
                                                        TJAPlayer3.Tx.SongSelect_Frame_Score.Opacity = (int)255.0f - (ctDifficultyIn.n現在の値 - 1000);
                                                    ...
                                                    TJAPlayer3.Tx.SongSelect_Frame_Score.t2D下中央基準描画(TJAPlayer3.app.Device, 487 + (i * 122) - 31, TJAPlayer3.Skin.SongSelect_Overall_Y + 463, new Rectangle(122 * i, 0, 122, 360));

                                                    #region [ 譜面分岐(かんたん～おに) ]

                                                    if (TJAPlayer3.Tx.SongSelect_Branch != null && TJAPlayer3.stage選曲.r現在選択中のスコア.譜面情報.b譜面分岐[0])
                                                        TJAPlayer3.Tx.SongSelect_Branch.t2D中心基準描画(TJAPlayer3.app.Device, 412, TJAPlayer3.Skin.SongSelect_Overall_Y + 273);

                                                    if (TJAPlayer3.Tx.SongSelect_Branch != null && TJAPlayer3.stage選曲.r現在選択中のスコア.譜面情報.b譜面分岐[1])
                                                        TJAPlayer3.Tx.SongSelect_Branch.t2D中心基準描画(TJAPlayer3.app.Device, 533, TJAPlayer3.Skin.SongSelect_Overall_Y + 273);

                                                    if (TJAPlayer3.Tx.SongSelect_Branch != null && TJAPlayer3.stage選曲.r現在選択中のスコア.譜面情報.b譜面分岐[2])
                                                        TJAPlayer3.Tx.SongSelect_Branch.t2D中心基準描画(TJAPlayer3.app.Device, 654, TJAPlayer3.Skin.SongSelect_Overall_Y + 273);

                                                    if (TJAPlayer3.Tx.SongSelect_Branch != null && TJAPlayer3.stage選曲.r現在選択中のスコア.譜面情報.b譜面分岐[3])
                                                        TJAPlayer3.Tx.SongSelect_Branch.t2D中心基準描画(TJAPlayer3.app.Device, 775, TJAPlayer3.Skin.SongSelect_Overall_Y + 273);
                                                ...
                                                if (i == 4 && !(ctBoxOpen.n現在の値 >= 1300 && ctBoxOpen.n現在の値 <= 1940))//ここが裏鬼部分？
                                                {
                                                    ...
                                                    t小文字表示(TJAPlayer3.stage選曲.r現在選択中のスコア.譜面情報.nレベル[4] < 10 ? 490 + (3 * 122) - 5 : 485 + (3 * 122) - 5, TJAPlayer3.Skin.SongSelect_Overall_Y + 275, TJAPlayer3.stage選曲.r現在選択中のスコア.譜面情報.nレベル[4].ToString());
                                                ...
                                                else if (i != 4 && !(ctBoxOpen.n現在の値 >= 1300 && ctBoxOpen.n現在の値 <= 1940))
                                                {
                                                    ...
                                                    t小文字表示(TJAPlayer3.stage選曲.r現在選択中のスコア.譜面情報.nレベル[i] < 10 ? 490 + (i * 122) - 5 : 485 + (i * 122) - 5, TJAPlayer3.Skin.SongSelect_Overall_Y + 275, TJAPlayer3.stage選曲.r現在選択中のスコア.譜面情報.nレベル[i].ToString());
                                            ...
                                            else //難易度がない場合
                                            {
                                                ...
                                                if (i == 4 && TJAPlayer3.stage選曲.n現在選択中の曲の難易度 == 4 && !(ctBoxOpen.n現在の値 >= 1300 && ctBoxOpen.n現在の値 <= 1940))
                                                {
                                                    ...
                                                    TJAPlayer3.Tx.SongSelect_Frame_Score.t2D下中央基準描画(TJAPlayer3.app.Device, 487 + (3 * 122) - 31, TJAPlayer3.Skin.SongSelect_Overall_Y + 463, new Rectangle(122 * 3, 0, 122, 360));
                                                }
                                                else if (i != 4 && !(ctBoxOpen.n現在の値 >= 1300 && ctBoxOpen.n現在の値 <= 1940))
                                                {
                                                    //鬼以下
            ...
            for (int i = 0; i < 9; i++)    // パネルは全13枚。
            {
                ...
                if ((i == 4) && (this.n現在のスクロールカウンタ == 0))
                {
                    ...
                    if (this.ttk選択している曲のサブタイトル != null)
                    {
                        ...
                        tx選択している曲のサブタイトル.t2D拡大率考慮中央基準描画(TJAPlayer3.app.Device, 640 + TJAPlayer3.Skin.SongSelect_Title_X, y + 94 - (this.stバー情報[nパネル番号].eバー種別 == Eバー種別.Box ? BarAnimeCount : BarAnimeCount / 1.1f) + TJAPlayer3.Skin.SongSelect_Title_Y);
                        if (this.ttk選択している曲の曲名 != null)
                        {
                            ResolveTitleTexture(this.ttk選択している曲の曲名).t2D拡大率考慮中央基準描画(TJAPlayer3.app.Device, 640 + TJAPlayer3.Skin.SongSelect_Title_X, y + 65 - (r現在選択中の曲.eノード種別 != C曲リストノード.Eノード種別.BACKBOX ? (this.stバー情報[nパネル番号].eバー種別 == Eバー種別.Box ? BarAnimeCount : BarAnimeCount / 1.1f) : 0) + TJAPlayer3.Skin.SongSelect_Title_Y);
                        }
                    }
                    else
                    {
        ...
        private void tジャンル別選択されていない曲バーの描画(int x, int y, string strジャンル, Eバー種別 eバー種別, int[] クリア, int[] スコアランク)
        {
            ...
            for (int i = 0; i < TJAPlayer3.Skin.SongSelect_Bar_Genre_Count; i++)
            {
                if (TJAPlayer3.stage選曲.act難易度選択画面.bIsDifficltSelect)
                {
                    ...
                    {
                        if (ctDifficultyIn.n現在の値 <= 1255)//難易度選択に入るときのフェード
                        {
                            ...
                            TJAPlayer3.Tx.SongSelect_Bar_Shadow.Opacity = (int)255.0f - (ctDifficultyIn.n現在の値 - 1000);
                            TJAPlayer3.Tx.SongSelect_Box_Shadow.Opacity = (int)255.0f - (ctDifficultyIn.n現在の値 - 1000);
                ...
                else
                {
                    ...
                    TJAPlayer3.Tx.SongSelect_Bar_Shadow.Opacity = (int)255.0f;
                    TJAPlayer3.Tx.SongSelect_Box_Shadow.Opacity = (int)255.0f;
            ...
            if (eバー種別 != Eバー種別.BackBox)
            {
                ...
                TJAPlayer3.Tx.SongSelect_Bar_Genre_Overlay?.t2D描画(TJAPlayer3.app.Device, x + 50, y + 17);
            }
            else
            {
                TJAPlayer3.Tx.SongSelect_Bar_Shadow?.t2D描画(TJAPlayer3.app.Device, x + 2, y + 2);
            ...
            {
                #region [ 現在選択されていない曲のスコア ]

                TJAPlayer3.Tx.SongSelect_Crown.vc拡大縮小倍率.X = 0.88f;//84
                TJAPlayer3.Tx.SongSelect_Crown.vc拡大縮小倍率.Y = 0.88f;
                TJAPlayer3.Tx.SongSelect_ScoreRank.vc拡大縮小倍率.X = 0.82f;//81
                TJAPlayer3.Tx.SongSelect_ScoreRank.vc拡大縮小倍率.Y = 0.82f;

                float Nsize = 42.80f;

                if (クリア[4] != 0)
                    TJAPlayer3.Tx.SongSelect_Crown.t2D拡大率考慮中央基準描画(TJAPlayer3.app.Device, x + 31, y + 30, new RectangleF(12 * 43.2f + (クリア[4] - 1) * 43.2f, 0, 43.2f, 39));
                else if (クリア[3] != 0 && クリア[4] == 0)
                    TJAPlayer3.Tx.SongSelect_Crown.t2D拡大率考慮中央基準描画(TJAPlayer3.app.Device, x + 31, y + 30, new RectangleF(9 * 43.2f + (クリア[3] - 1) * 43.2f, 0, 43.2f, 39));
                else if (クリア[2] != 0 && クリア[3] == 0)
                    TJAPlayer3.Tx.SongSelect_Crown.t2D拡大率考慮中央基準描画(TJAPlayer3.app.Device, x + 31, y + 30, new RectangleF(6 * 43.2f + (クリア[2] - 1) * 43.2f, 0, 43.2f, 39));
                else if (クリア[1] != 0 && クリア[2] == 0)
                    TJAPlayer3.Tx.SongSelect_Crown.t2D拡大率考慮中央基準描画(TJAPlayer3.app.Device, x + 31, y + 30, new RectangleF(3 * 43.2f + (クリア[1] - 1) * 43.2f, 0, 43.2f, 39));
                else if (クリア[0] != 0 && クリア[1] == 0)
                    TJAPlayer3.Tx.SongSelect_Crown.t2D拡大率考慮中央基準描画(TJAPlayer3.app.Device, x + 31, y + 30, new RectangleF(0 * 43.2f + (クリア[0] - 1) * 43.2f, 0, 43.2f, 39));


                if (スコアランク[4] != 0)
                    TJAPlayer3.Tx.SongSelect_ScoreRank.t2D拡大率考慮中央基準描画(TJAPlayer3.app.Device, x + 32, y + 64, new RectangleF(0, (スコアランク[4] - 1) * Nsize, 50, Nsize));
                else if (スコアランク[3] != 0 && スコアランク[4] == 0)
                    TJAPlayer3.Tx.SongSelect_ScoreRank.t2D拡大率考慮中央基準描画(TJAPlayer3.app.Device, x + 32, y + 64, new RectangleF(0, (スコアランク[3] - 1) * Nsize, 50, Nsize));
                else if (スコアランク[2] != 0 && スコアランク[3] == 0)
                    TJAPlayer3.Tx.SongSelect_ScoreRank.t2D拡大率考慮中央基準描画(TJAPlayer3.app.Device, x + 32, y + 64, new RectangleF(0, (スコアランク[2] - 1) * Nsize, 50, Nsize));//42.71
                else if (スコアランク[1] != 0 && スコアランク[2] == 0)
                    TJAPlayer3.Tx.SongSelect_ScoreRank.t2D拡大率考慮中央基準描画(TJAPlayer3.app.Device, x + 32, y + 64, new RectangleF(0, (スコアランク[1] - 1) * Nsize, 50, Nsize));
                else if (スコアランク[0] != 0 && スコアランク[1] == 0)
                    TJAPlayer3.Tx.SongSelect_ScoreRank.t2D拡大率考慮中央基準描画(TJAPlayer3.app.Device, x + 32, y + 64, new RectangleF(0, (スコアランク[0] - 1) * Nsize, 50, Nsize));
